...
- `economy_coin_min`: 成長時の所持Cが `value` 以上

条件を満たす候補がなかった場合にも同じ形態へ進化させるときは、`evolution` に `"fallback": true` を追加します。条件を満たした場合は通常の優先順位で判定され、どの候補にも一致しなかった場合はフォールバックとして選ばれます。
